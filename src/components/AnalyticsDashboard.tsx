...
const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ data }) => {
  ...
  const companyData = Object.entries(data.byCompany)
    .sort((a, b) => b[1] - a[1])
  ...
  const monthlyData = Object.entries(data.applicationsByMonth)
    .map(([name, value]) => ({
      name,
      applications: value
    }))
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription>Percentage of applications reaching interview stage</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-4xl font-bold">{data.interviewRate.toFixed(1)}%</div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex flex-col items-center justify-center p-4 bg-background rounded-lg border">
              ...
              <span className="text-2xl font-bold">{data.responseRate.toFixed(1)}%</span>
            </div>
            <div className="flex flex-col items-center justify-center p-4 bg-background rounded-lg border">
              <span className="text-muted-foreground text-sm">Interview Rate</span>
              <span className="text-2xl font-bold">{data.interviewRate.toFixed(1)}%</span>
            </div>
            <div className="flex flex-col items-center justify-center p-4 bg-background rounded-lg border">
              <span className="text-muted-foreground text-sm">Offer Rate</span>
              <span className="text-2xl font-bold">{data.offerRate.toFixed(1)}%</span>
            </div>
            {data.averageResponseTime && (
              <div className="flex flex-col items-center justify-center p-4 bg-background rounded-lg border">
                <span className="text-muted-foreground text-sm">Avg. Response Time</span>
                <span className="text-2xl font-bold">{data.averageResponseTime.toFixed(1)} days</span>
              </div>
            )}
          </div>
        </CardContent>
